...
#include "..\\certdll\\CertDll.h"
#if _DEBUG
#pragma comment (lib, "..\\debug\\certdll.lib")
#else
#pragma comment (lib, "..\\release\\certdll.lib")
#endif
#include "NPString.h"
                               ...
                               uint32_t argCount, NPVariant *result)
{
 ...
 // char* name = npnfuncs->utf8fromidentifier(methodName);
  ...
  if (!strcmp(pFunc, "FindCertByDN"))
  {
	...
	CNPString DN(args[0].value.stringValue);
	MsgBox(DN);

	BOOL nRet = FindCertByDN("MY", DN);
	MsgBox(nRet);
  ...
  else if (!strcmp(pFunc, "CreateCSR"))
  {
	  char csr[4096];
	  memset(csr, '\0', sizeof(csr));

	  int nRet = CreateCSR(csr);
    ...
    if (!npOutString)
      return false;
    strcpy(npOutString, csr);

	  STRINGZ_TO_NPVARIANT(npOutString, *result);

	  return true;
  }
  else if(!strcmp(pFunc, "InstallCert"))
  {
	  if (argCount < 1)
		  return false;

		CNPString cert(args[0].value.stringValue);
		
		BOOL bRet = InstallCert(cert);

		INT32_TO_NPVARIANT(bRet, *result);
		return true;
  }
  else if(!strcmp(pFunc, "Sign"))
  {
		if (argCount < 2)
			return false;
	...
	CNPString RawData(args[1].value.stringValue);
	
	char SignResult[4096];
	  memset(SignResult, '\0', sizeof(SignResult));
	int nRet = Sign(DN, RawData, SignResult);
	  ...
	  int len = strlen(SignResult);
	  MsgBox(len);

	  char* npOutString = (char *)NPN_MemAlloc(len + 1);
    if (!npOutString)
      return false;
    strcpy(npOutString, SignResult);

	  STRINGZ_TO_NPVARIANT(npOutString, *result);
  }
  else if(!strcmp(pFunc, "DeleteCert"))
  {
	  if (argCount < 1)
		  return false;
